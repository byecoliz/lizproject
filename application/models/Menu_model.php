...
class Menu_model extends CI_Model
{
    ...
    function waiting_list_count()
    {
        ...
        $this->db->where('PATIENT', $this->session->userdata('username'));
    ...
    {
        $this->db->select("wl.PATIENT,wl.STATUS_OF_WAITING_LIST,wl.CLINICAL_DEPARTMENT,h.WL_HOSPITALIZATION_DATE,
...
DATEDIFF(h.WL_HOSPITALIZATION_DATE,wl.WAITING_LIST_DATE) AS Duration", false);
        ...
        $this->db->where('wl.patient', $this->session->userdata('username'));
        $q = $this->db->get();
        if ($q->num_rows() > 0) {
            foreach ($q->result() as $row) {
                $data[] = $row;
